::  by JaCk  |  Release 01/17/2018  |  https://github.com/1ijack/BatchMajeek/md5.cmd  |  md5.cmd - compute/[verify(manually)] MD5 hash of a file using PowerShell (or almost any other tool).  (Verify, is not working atm, although I just coded it in another similar script)
::: 
...
:func_md5_a_help
    ...
    if not defined emde5_recurse_dirs ( echo/                           - Default Recursion: disable) else ( echo/                           - Default Recursion: %emde5_recurse_dirs%)
    ...
    echo/          false, off,
    echo/                 -rd       - Additional arg, specificly for disable 
    echo/
    ...
    echo/    --ud, --utf8-disable,  - Additional args specificly for disable 
    echo/    --unicode-disable      - Additional args specificly for disable 
...
:func_md5_args_and_input
    if defined emde5_unicode call :func_md5_encoding_set
    ...
    for %%Z in (
        ...
        "%%X"
                           ...
                           if "-%%~V" equ "-enable"     call :func_md5_encoding_set
                           if "-%%~V" equ "-true"       call :func_md5_encoding_set
                           if "-%%~V" equ "-on"         call :func_md5_encoding_set
                           if "-%%~V" equ "-"           call :func_md5_encoding_set
...
:func_md5_basic_errorcheck
    ...
    if "%_emd5_err_ac_:~0,1%" equ "d" (
        set "_emd5_err_ac_="
        exit /b 10
    ) else set "_emd5_err_ac_="
    
    rem  Make sure "user" neq "troll"
    if exist "%~1" exit /b 0
    ( for /f "tokens=* delims=" %%A in ('dir /b /a "%~1"') do if "%%~A" neq "" exit /b 0
    ) 1>nul 2>nul 
...
::  Usage  ::  func_md5_encoding_set   optional-EncodingChcpNumber
...
:func_md5_encoding_set
    if "%~1" equ "" call %~0   65001
    if "%~1" equ "" goto :eof
    
    ( if not defined emde5_chbin for /f "delims=" %%C in ('"%SystemRoot%\System32\where.exe" "%SystemRoot%\System32:chcp.com" "%path%:chcp.com"') do if "%%~C" neq "" set "emde5_chbin=%%~C"
    ) 2>nul 1>nul
    if not defined emde5_chbin exit /b 1

    if not defined emde5_chcp_orig for /f "tokens=1,4 delims=: " %%C in ('"%emde5_chbin%"') do if /i "%%~C" equ "Active" if "%%~D" equ "" (
        echo/ Error: chcp: unexpected or missing query output: "%%~C %%~D"
        echo/ Error: chcp: Unable to change console encoding.
        echo/ Info: chcp: If utf8 support is necessary, run the command manually before you start this script:
        echo/ Info: chcp: command: "%emde5_chbin%" %~1 
        set "emde5_chbin="
        exit /b 1
    ) 1>&2 else set "emde5_chcp_orig=%%~D"

    if "%emde5_chcp_orig%" neq "%~1" for /f "tokens=1,4 delims=: " %%C in ('"%emde5_chbin%" %~1') do if "%%~D" neq "%~1" (
        echo/ Error: chcp: UTF8 Support unavailible
        if /i "%%~C" neq "Active" (
            echo/ Error: chcp: unexpected output: "%%~C %%~D"
            echo/ Info: chcp: command: "%emde5_chbin%" %~1 
        ) 1>&2 

        chcp %emde5_chcp_orig%
        set "emde5_chcp_orig="
        set "emde5_chbin="
        exit /b 1
    ) 1>&2 else set "emde5_unicode=true"
...
rem    All params are ignored and unused
:func_md5_encoding_unset
    if not defined emde5_chcp_orig goto :eof

    for /f "tokens=1,4 delims=: " %%C in ('"%emde5_chbin%" %emde5_chcp_orig%') do if "%%~D" equ "%emde5_chcp_orig%" (
        set "emde5_chbin="
        set "emde5_unicode="
        set "emde5_chcp_orig="
    ) 
...
:func_md5_wait_check 
    rem  check literacy\compentency; talk to self when ritard
    if "%~1" equ "" call "%~0"  "%~nx0"
    if "%~1" equ "" goto :eof
    ...
    if 9%errorlevel% neq 90 goto :eof

    rem  check loosely -- scriptname neq cmdlaunchname;  when true, trigger wait
    ( echo/"%CMDCMDLINE%" 
    ) | findstr /vi "%~nx1" 2>nul 1>nul
    if 9%errorlevel% equ 91 timeout /t 60
