...
:func_zk_arg_shifter
    if "%~1" equ "" goto :func_zk_zend
    set /a "zk_param_count+=1"
    set "#func_zk_arg_shifter{One}=%~1"
    set "#func_zk_arg_shifter{One{a}}=%~a1"

    rem  cannot be empty; sets to ' ' when empty
    if "%#func_zk_arg_shifter{One{a}}%" equ "" set "#func_zk_arg_shifter{One{a}}= "
    ...
    if "%#func_zk_arg_shifter{One{a}}:~0,1%" equ "d" if /i "%zk_print_script_msgs%" equ "true"             call :func_zk_remove_empties   "%~1"
    if "%#func_zk_arg_shifter{One{a}}:~0,1%" equ "d" if /i "%zk_print_script_msgs%" neq "true" 2>nul 1>nul call :func_zk_remove_empties   "%~1" 
    shift /1
    if "%#func_zk_arg_shifter{One{a}}:~0,1%" equ "d" goto :func_zk_arg_shifter
...
:func_zk_daFifth
    ...
    rem  when the next arg is not empty leave func
...
:func_zk_remove_empties
    ...
    set "zk_empty_dir_count=0"
    set "zk_empty_file_count=0"
