...
rem  semi-colon separated list of processes
 set "afu_process_list=igfxEM.exe;igfxHK.exe;igfxTray.exe;atbroker.exe;tabtip.exe"
...
rem  dynamic, local file with a process list ---- conf syntax - a list (space,comma,semi-colon delimited)
 ...
 set "afu_list_add_unvarred_keys=true"       rem Set to [true]  if you want to have [afu_list_conf] add alienated keys as processnames to watch; 
    rem Example when true:  $0  would see "mydumb.exe" and "other dump.exe" as processnames, but still see "afu_process_list" as a list label
