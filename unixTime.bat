::  by JaCk  |  Release 05/11/2018  |   https://github.com/1ijack/BatchMajeek/blob/master/unixTime.bat  |  unixTime.bat  --  returns current time in unixtime  --  total elapsed seconds since January 1st, 1970
:::
...
:::
@echo off & setlocal DisableDelayedExpansion EnableExtensions & for %%A in (utx_utc_offset;utx_out_ms;utx_leap{total};utx_utc{shift};utx_time{unix}) do set "%%A="
...
(echo/%utx_time{unix}%)& for %%A in (utx_utc_offset;utx_out_ms;utx_leap{total};utx_utc{shift};utx_time{unix}) do set "%%A="
